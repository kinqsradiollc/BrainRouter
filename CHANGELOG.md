...
|---|---|---|
| **0.4.15** | On `release/0.4.15` — **The requirement-first workflow + the unified workspace** (requirement→plan→task→review→verify→memory, all provenance-tracked) plus **Chat · Track · Code** modes from a left-sidebar switcher — Track is a code-aware, Jira-class board per workspace (Board/List/Backlog/Sprint/Roadmap/Reports/Automation/Members/Sync · JQL queries · automation rules · per-project roles · GitHub Issues sync) and Chat is a read-only conversation — plus a desktop UI pass (Graphite Mono theme · colour-toned tool rows with clickable paths · end-of-turn changeset card · live Context panel with token + cache/compaction/memory savings · one-column layout) and a steadier core (bounded reranker/embedding timeouts + circuit breaker, non-blocking capture, strict tool-call pairing fix for the 2013 error, injection-surface + tenancy hardening, CLI shim removal) | [`brainrouter-changelog/0.4.15.md`](brainrouter-changelog/0.4.15.md) |

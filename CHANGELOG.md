...
- **`ask_user_choice` local tool** — mid-turn the agent can pause, present 2–4 mutually exclusive options to the user, and resume with the chosen label (or array of labels when `multiSelect` is true). Input is validated as an option number OR a case-insensitive label match; ambiguous prefixes are explicitly refused (the helper never silently picks one for the user). Non-interactive runs (CI, piped, `brainrouter run`) surface a `NoTTYError` instead of defaulting to option 1, so the agent falls back to deciding itself with full context rather than silently committing the user to a path. Mirrors `askYesNo`'s `activeReadline` bridge, so the parent REPL is paused/resumed cleanly around the prompt. Tool name is `ask_user_choice`; the system prompt now scopes its use to "genuine ambiguity with 2–4 mutually exclusive reasonable approaches" — explicitly not a substitute for `askYesNo` confirmations or for thinking.

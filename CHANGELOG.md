...
- **REPL stays responsive after slash commands that show a spinner.** `/working`, `/handover`, `/explain`, `/diagnostics`, `/forget`, `/persona`, `/skill-hints` (anything going through `printMcpCall` / `ora`) used to leave the REPL stuck — arrow keys echoed `^[[A` literally, Backspace echoed `^?`, the prompt looked live but ignored every keystroke. Root cause: `ora` depends on `stdin-discarder`, which toggles `setRawMode(false)` + `stdin.pause()` when the spinner stops; the slash-command line handler didn't bracket dispatch in `rl.pause()`/`rl.resume()` the way `runAgentTurn` does, so the parent REPL inherited the cooked-mode + paused state. Fixed by mirroring the bracket and guarding the resume against `isProcessing === true` so handlers that hand off to a turn don't race two resumes.

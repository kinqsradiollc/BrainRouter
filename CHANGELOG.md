...
|---|---|---|
| **0.4.14** | Release branch — pending publish — **Recall you can trust** (benchmark-driven recall overhaul in two rounds: chunked import + length-aware rerank caps, then score-blend/reorder-never-drop across reranker+judge; LongMemEval R-any@5 0.63→0.87, LoCoMo 0.83→0.90, reflective latency ~9–18s→~0.06–6.6s) + **the grid TUI** (view router · context sidebar with live Sub-agents/Workers/Workflows fleet · scrollback fit + scroll mode · live timers) + **background workers that report back** (model-spawned worker threads, depth-gated; completion inbox feeds detached results into the next turn) | [`brainrouter-changelog/0.4.14.md`](brainrouter-changelog/0.4.14.md) |

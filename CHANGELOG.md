...
|---|---|---|
| **0.4.20** | In progress — security + correctness pass: deep review restored (git was absent from the brain image), the `cleanup` stage CHECK that silently killed every PR review, an interrupted assurance stage made retryable, a quadratic webhook regex that stalled the shared brain ~23s per PR comment, three provider-probe SSRFs closed behind the DNS-pinned upstream policy, `config.json` written `0600` with an injectable path, revocable refresh sessions with theft detection and per-account sign-in throttling, `connector_sync` ENOENT, project-scoped learned lessons, seconds-scale durable meeting chunks with corruption-safe Dashboard/Desktop recovery, and every non-breaking dependency upgrade | [`brainrouter-changelog/0.4.20.md`](brainrouter-changelog/0.4.20.md) |

...
|---|---|---|
| **0.4.22** | In development — a simple, native Desktop workbench with a task-led start surface across Chat, Code, Track, panels, and Settings; every mode now states its scope and transition consequence, cross-mode workspace links reach their owning surface, active side panels and live assessments avoid hidden or overlapping refreshes, and memory recall starts independent candidate retrieval concurrently | [`brainrouter-changelog/0.4.22.md`](brainrouter-changelog/0.4.22.md) |
| **0.4.21** | Qualified, unpublished — plug-and-play runtime (the MCP + builtin-tool switches dissolved into registries; capability ports + execution worlds), the glass box (a trajectory ledger, request inspector, and composition panel that let a human watch, inspect, and replay the process), the live edge egress tunnel with per-org consent + kill-switch + telemetry, the completed credential-hardening program (no credential in browser storage), agent worktrees, and a batch of closed provider-SSRF / ReDoS security holes | [`brainrouter-changelog/0.4.21.md`](brainrouter-changelog/0.4.21.md) |
| **0.4.20** | Shipped — security + correctness pass plus accepted ADR-034 exact-key session messaging across Core, Brain, CLI, and Desktop with authenticated Brain-offline delivery, collision-safe state, recipient approval, absolute expiry, and tenant-pinned durable wake/poll recovery; accepted ADR-038 with one shared Planner/Notes UI, a real working-day Planner, durable sync, authenticated lease-gated Notes editing, canonical CLI `/planner`, PostgreSQL multi-device/rollback gates, and blocking cross-host visual checks; also fixes all-profile workspace/plan orchestration identity, requires exact single-use host intent on the production Agent workflow-launch path, restores deep review, fixes the review-stage CHECK and retry lifecycle, closes provider-probe SSRFs, adds revocable refresh sessions and account throttling, fixes connector sync, scopes learned lessons, adds seconds-scale durable meeting chunks with corruption-safe Dashboard/Desktop recovery, and takes every non-breaking dependency upgrade | [`brainrouter-changelog/0.4.20.md`](brainrouter-changelog/0.4.20.md) |
| **0.4.18** | Shipped — 2026-08-01 — native Desktop appearance, unified workbench surfaces, persistent panel state, faster Files/Editor navigation, and phase-aware plan execution | [`brainrouter-changelog/0.4.18.md`](brainrouter-changelog/0.4.18.md) |
| **0.4.17** | Shipped — 2026-07-30 — conversational workspace setup, profile-aware agent runtime, stable revisioned work identity, scoped specialist packs, and Project knowledge from bounded ingest through citation-backed Dashboard/Desktop workflows | [`brainrouter-changelog/0.4.17.md`](brainrouter-changelog/0.4.17.md) |
| **0.4.16** | Shipped — 2026-07-01 — **The autonomous fleet, the unified desktop, and a Postgres brain** — an autonomous **Honk fleet** (durable global job queue + single-runner host lock · recipe/build fan-out executors that codemod-then-verify each repo as an isolated PR · `brainrouter fleet` run/status/drain + an MCP fleet surface + dashboard console · PR-from-a-green-build-loop `cli.buildLoopEmitPr` · un-opt-out-able sandbox + secret-env scoping · an L1–L7 local-model bounded harness); a **unified desktop workspace** where Chat·Track·Code become full modes (Write W1–W6 Monaco/diff/ghost-text/DOC-HTML export · sandboxed prototype generation D2–D4 · a `run_workflow_graph` visual canvas L1–L4 · a Memory panel · system tray · rebindable shortcuts · @-mention file picker · Settings→Tools per-tool policy · signed builds); **Track** matured into a Jira/Plane-class board (lifecycle model · multi-assignee + label registry · Modules + relations · calendar/gantt/spreadsheet layouts + saved Views · **bidirectional 3-way GitHub Issues sync** that merges without clobbering); the brain cut over to **PostgreSQL + pgvector** behind a fully async engine (SQLite removed · tunable ivfflat/hnsw index · pg in CI) with a **remote-brain runtime** (`cli.brainUrl` · Atlas-as-service · cloud packaging) and an **Atlas** codebase knowledge graph (scanner + LLM enrich + structural/services views + impact/coverage overlays + symbol line-jump); plus a **`packages/core` public-API boundary** (curated barrels · machine-enforced import rule), native Anthropic/Gemini wire adapters (opt-in), Fast decoupled from reasoning, today's-date in Runtime Context, a dashboard await fix, and a connector-honesty pass | [`brainrouter-changelog/0.4.16.md`](brainrouter-changelog/0.4.16.md) |
| **0.4.15** | Shipped — 2026-06-22 — **The requirement-first workflow + the unified workspace** (requirement→plan→task→review→verify→memory, all provenance-tracked) plus **Chat · Track · Code** modes from a left-sidebar switcher — Track is a code-aware, Jira-class board per workspace (Board/List/Backlog/Sprint/Roadmap/Reports/Automation/Members/Sync · JQL queries · automation rules · per-project roles · GitHub Issues sync) and Chat is a read-only conversation — plus a desktop UI pass (Graphite Mono theme · colour-toned tool rows with clickable paths · end-of-turn changeset card · live Context panel with token + cache/compaction/memory savings · one-column layout), a closing **desktop UX & reliability pass** (idle child auto-resume · destructive-command guard · MCP auto-reconnect + brain/tools split · provider gallery with live `/models` · cross-session usage heatmap · stop-workflow · rewind · cross-workspace session menu), and a steadier core (bounded reranker/embedding timeouts + circuit breaker, non-blocking capture, strict tool-call pairing fix for the 2013 error, injection-surface + tenancy hardening, CLI shim removal) | [`brainrouter-changelog/0.4.15.md`](brainrouter-changelog/0.4.15.md) |
| **0.4.14** | Shipped — 2026-06-10 — **Recall you can trust** (benchmark-driven recall overhaul in two rounds: chunked import + length-aware rerank caps, then score-blend/reorder-never-drop across reranker+judge; LongMemEval R-any@5 0.63→0.87, LoCoMo 0.83→0.90, reflective latency ~9–18s→~0.06–6.6s) + **the grid TUI** (view router · context sidebar with live Sub-agents/Workers/Workflows fleet · scrollback fit + scroll mode · live timers) + **background workers that report back** (model-spawned worker threads, depth-gated; completion inbox feeds detached results into the next turn) | [`brainrouter-changelog/0.4.14.md`](brainrouter-changelog/0.4.14.md) |
| **0.4.13** | Shipped — 2026-06-07 — **Sub-agent results that come back** (completed sub-agents / workflows now flow back to the main agent as a synthesis continuation — poll + event-driven resume; weak-model synthesis guard; "is it done?" id-correlation) + **REPL polish** (parallel same-name tool-result pairing; `file:line` citation repair; low-effort next-action planner) | [`brainrouter-changelog/0.4.13.md`](brainrouter-changelog/0.4.13.md) |
| **0.4.12** | Shipped — 2026-06-05 — **The Build Loop** (`/build` plan→implement→verify→review→merge; phase-scoped shared worktree + merge gate; `cli.buildLoop` escalation; fan-out builds + `cli.worktreeMergeReview`; opt-in loop-until-green `cli.buildLoopMaxRepairs`; active phase in `/ps`·`/agents`·statusline) + **multi-agent reliability** (parent loop auto-resumes timed-out children; `/queue` input queue; reconnect-with-backoff for model + memory calls instead of hard timeouts; child/worker timeouts wait but never kill; live-worktree-safe GC) + **accuracy** (real-regex `grep_search`; `/tokens` includes child tokens; graceful role degradation) | [`brainrouter-changelog/0.4.12.md`](brainrouter-changelog/0.4.12.md) |
...
| **0.4.7** | Shipped — 2026-06-02 | [`brainrouter-changelog/0.4.7.md`](brainrouter-changelog/0.4.7.md) |
| **0.4.6** | Shipped — 2026-06-01 | [`brainrouter-changelog/0.4.6.md`](brainrouter-changelog/0.4.6.md) |

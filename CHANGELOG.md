...
- **Cognitive extractor no longer drops a whole batch over one bad escape or corrupts path escapes during repair.** `parseExtractionResult` now goes through `parseJsonWithEscapeRepair`: on `SyntaxError` it doubles ambiguous backslashes and retries the parse. Windows paths, LaTeX (`\section`), regex literals, and shell snippets in `content` fields no longer cause the extractor to log `Bad escaped character in JSON at position …` and discard every memory in the batch; path segments such as `\bin`, `\target`, and `\release` also remain literal instead of being parsed as control characters. ([`brainrouter/src/memory/pipeline/cognitive-extractor.ts`](brainrouter/src/memory/pipeline/cognitive-extractor.ts))

...
test('serveStartupWarnings surfaces empty allowlist + missing secret (never fatal)', () => {
  const bare = serveStartupWarnings(knobs({ enabled: true }), { github: '', slack: '' });
  ...
  const ready = serveStartupWarnings(knobs({ enabled: true, allowedRepos: ['acme/*'] }), { github: 's3cret', slack: 's3cret' });

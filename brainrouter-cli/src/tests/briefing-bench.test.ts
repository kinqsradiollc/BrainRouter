...
import { buildMemoryBriefing } from '@kinqs/brainrouter-core/memory';
...
test('briefing bench: cli.personaAnchor=off suppresses persona from default plan', async () => {
  const { buildDefaultSourcePlan, describeSourcePlan } = await import('@kinqs/brainrouter-core/memory');
...
test('briefing bench: personaAnchorPreference=false overrides config=on', async () => {
  const { buildDefaultSourcePlan } = await import('@kinqs/brainrouter-core/memory');

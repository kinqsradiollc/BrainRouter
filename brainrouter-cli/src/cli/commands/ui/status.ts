/**
 * Extracted from cli/commands/ui.ts — diagnostics / status subcommands.
 *
 * `/status`, `/workspace`, `/policy`, `/doctor`. Behavior-preserving:
 * bodies moved verbatim; the dispatcher keeps the "return true iff the
 * command matched" contract so ordering across ui/* handlers is a no-op.
 */
...
import { getConfigPath, setCliKnobOverride, getCliKnobs } from '@kinqs/brainrouter-core/config';

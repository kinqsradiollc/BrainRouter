...
- **openSrc/ Reference Habit**: If the optional `openSrc/` folder is present, **read [`openSrc/REFERENCES.md`](openSrc/REFERENCES.md) FIRST** — it is the router for every vendored project under that path. Follow its rules: don't grep `openSrc/` by default (search BrainRouter's own tree first); when you do need an external pattern, scope greps to a single project's `src/` (not the whole folder); skip marketing material (READMEs, banners, `RELEASE_v*.md`); and cite the project + file path in a code comment whenever you adapt an idea. The quick-lookup table at the bottom of REFERENCES.md tells you which project to consult for which kind of question.
...
- **`openSrc/`** *(Conditional, gitignored)*: Vendored open-source projects we read for ideas, comparison, and concrete implementation patterns. **Read [`openSrc/REFERENCES.md`](openSrc/REFERENCES.md)** for the project index, when-to-consult guidance, and the quick-lookup table. Nothing under `openSrc/` is built or imported by BrainRouter — it is research material only.
...
3. Review `openSrc/` patterns if relevant — start at [`openSrc/REFERENCES.md`](openSrc/REFERENCES.md) (project index + quick-lookup table), then drill into the named project's `src/`. Do NOT grep across `openSrc/` blindly; pick a project deliberately.

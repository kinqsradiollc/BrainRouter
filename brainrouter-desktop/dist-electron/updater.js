...
export async function initAutoUpdate(opts = {}) {
    ...
    try {
        ...
        console.warn('[updater] BRAINROUTER_UPDATE_CHANNEL is set but electron-updater is not installed — ' +
    ...
    try {
        ...
        autoUpdater.on('download-progress', (p) => emit({ kind: 'progress', percent: Math.round(p?.percent ?? 0) }));

 ...
 */
export function shouldIgnoreWatchPath(rel) {
    if (!rel)
        return false;
    const norm = rel.split(path.sep).join('/');
    if (norm.split('/').some((seg) => IGNORE_DIRS.has(seg)))
        return true;
    if (IGNORE_FILE.test(norm))
        return true;
    return false;
}
/**
 ...
 */
export function startWorkspaceWatcher(root, onChange, debounceMs = 300) {
    let timer = null;
    const schedule = () => {
        if (timer)
            clearTimeout(timer);
        timer = setTimeout(() => { timer = null; try {
            onChange();
        }
        catch { /* swallow */ } }, debounceMs);
    };
    try {

 ...
 */
export function planActivate(state, workspaceRoot, now, ttlMs = DEFAULT_IDLE_TTL_MS) {
    const exists = state.entries.some((e) => e.workspaceRoot === workspaceRoot);
    const reap = state.entries
        .filter((e) => e.workspaceRoot !== workspaceRoot && // never the one we're switching to
...
export function applyActivate(state, plan, now) {
    ...
    if (state.activeRoot && state.activeRoot !== plan.activeRoot) {
        entries = entries.map((e) => e.workspaceRoot === state.activeRoot ? { ...e, lastActiveAt: now } : e);
    }
    if (!entries.some((e) => e.workspaceRoot === plan.activeRoot)) {
        ...
        entries = entries.map((e) => e.workspaceRoot === plan.activeRoot ? { ...e, lastActiveAt: now } : e);
    }
    return { entries, activeRoot: plan.activeRoot };
}
/** Mark a workspace's host busy/idle (driven by turn-start / turn-end events). */
export function setRunning(state, workspaceRoot, running) {
    return {
        ...state,
        entries: state.entries.map((e) => e.workspaceRoot === workspaceRoot ? { ...e, running } : e),
    };
}
/** Remove a host (e.g. its process exited) from the pool. */
export function removeEntry(state, workspaceRoot) {
    return {
...
export function runningRoots(state, exclude) {

...
function activeWindow(windowProvider) {
...
function toNativePoint(x, y, win) {
    const display = displayForWindow(win);
    const scale = display.scaleFactor || 1;
    return { x: Math.round(x * scale), y: Math.round(y * scale) };
}
async function withHiddenWindow(win, fn) {
...
async function screenshot(windowProvider) {
...
function permissionDeniedFor(action) {
...
async function key(libnut, action) {
    if (!action.keys)
        return;
    ...
    if (primary)
        libnut.keyTap(primary, modifiers);
}
export function createComputerUsePort(windowProvider) {
    return {
        async screenshot() {
        ...
        async act(action) {
            ...
            if (!loaded.ok)
                return { success: false, permissionDenied: true, error: loaded.error };
            const win = activeWindow(windowProvider);
            try {

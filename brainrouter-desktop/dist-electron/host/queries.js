...
export function buildQueries(ctx) {
    ...
    return {
        ...
        'track-sync': async (a) => {
            ...
            if (!repo)
                return { error: oauth ? 'No GitHub repository detected for this workspace — open a folder with a GitHub remote.' : 'No repository configured. Configure a GitHub connector in Settings → Connectors → GitHub.' };
            ...
            const proxyFetch = (base, apiKey) => async (url, init) => {
                const u = new URL(url);
            ...
            if (oauth) {
                const opts = { repo, token: 'via-oauth-broker', fetchImpl: proxyFetch(oauth.base, oauth.apiKey), dryRun };
                if (direction === 'export')
            ...
            if (!cfg.token)
                return { error: 'Connect GitHub in Settings → Connectors → GitHub (recommended), or add a token / set GITHUB_TOKEN.' };
        ...
        'reviews': async () => {
            ...
            const apiKey = bId ? String(cfg.servers[bId]?.apiKey ?? '') : '';
            if (!base || !apiKey)
                return { signedIn: false, reviews: [] };
            try {
                const r = await fetch(`${base}/api/admin/reviews/jobs?limit=40`, { headers: { Authorization: `Bearer ${apiKey}` } });
                if (!r.ok)
                    return { signedIn: true, reviews: [], error: r.status === 403 ? 'Owner/admin only' : `HTTP ${r.status}` };
                const j = await r.json();
                return { signedIn: true, reviews: Array.isArray(j.reviews) ? j.reviews : [] };
            }
            catch (e) {
                return { signedIn: true, reviews: [], error: e instanceof Error ? e.message : 'fetch failed' };
            }
        },
        ...
        'github-connect-status': async () => {
            ...
            try {
                ...
                if (!r.ok)
                    return { signedIn: true, error: `HTTP ${r.status}` };
                return { signedIn: true, ...await r.json() };
            }
            catch (e) {
                return { signedIn: true, error: e instanceof Error ? e.message : 'failed' };
            }
        },
        ...
        'action:github-disconnect': async () => {
            ...
            if (!base || !apiKey)
                return { ok: false };
            try {
                await fetch(`${base}/api/connectors/github/disconnect`, { method: 'POST', headers: { Authorization: `Bearer ${apiKey}` } });
                return { ok: true };
            }
            catch {
                return { ok: false };

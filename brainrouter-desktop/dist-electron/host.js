...
import { buildBaseGraph, saveAtlasGraph, readAtlasGraph, atlasGraphStats, enrichAtlasGraph } from '@kinqs/brainrouter-core/dist/atlas/index.js';
 ...
 */
async function fetchEndpointModels(endpoint, apiKey) {
    ...
    const core = createHostCore({
        ...
        queries: {
            ...
            'git-pr-detail': async () => new Promise((resolve) => {
                execFile('gh', ['pr', 'view', '--json', 'number,state,title,url,headRefName,baseRefName,author,isDraft,mergeable,statusCheckRollup'], { cwd: workspaceRoot, timeout: 8_000, maxBuffer: 2_000_000 }, (err, stdout) => {
                    if (err) {
                        resolve({ pr: null });
                        return;
                    }
                    try {
                        resolve({ pr: JSON.parse(stdout) });
                    }
                    catch {
                        resolve({ pr: null });
                    }
            ...
            'git-pr-checks': async () => new Promise((resolve) => {
                ...
                execFile('gh', ['pr', 'checks', '--json', 'name,state,bucket,link,workflow,startedAt,completedAt'], { cwd: workspaceRoot, timeout: 8_000, maxBuffer: 2_000_000 }, (_err, stdout) => {
                    try {
                        resolve({ checks: JSON.parse(stdout) });
                    }
                    catch {
                        resolve({ checks: [] });
                    }
            ...
            'git-actions-runs': async (args) => new Promise((resolve) => {
                ...
                execFile('gh', ['run', 'list', '--limit', String(limit), '--json', 'databaseId,name,displayTitle,status,conclusion,workflowName,headBranch,event,createdAt,url'], { cwd: workspaceRoot, timeout: 9_000, maxBuffer: 4_000_000 }, (err, stdout) => {
                    if (err) {
                        resolve({ runs: [] });
                        return;
                    }
                    try {
                        resolve({ runs: JSON.parse(stdout) });
                    }
                    catch {
                        resolve({ runs: [] });
                    }
            ...
            'git-actions-run-detail': async (args) => new Promise((resolve) => {
                ...
                execFile('gh', ['run', 'view', id, '--json', 'databaseId,name,displayTitle,status,conclusion,jobs,workflowName,headBranch,url,createdAt'], { cwd: workspaceRoot, timeout: 9_000, maxBuffer: 4_000_000 }, (err, stdout) => {
                    if (err) {
                        resolve({ run: null });
                        return;
                    }
                    try {
                        resolve({ run: JSON.parse(stdout) });
                    }
                    catch {
                        resolve({ run: null });
                    }
            ...
            'action:git-actions-rerun-failed': async (args) => new Promise((resolve) => {
                const id = String(args.id ?? '').replace(/[^0-9]/g, '');
                if (!id) {
                    resolve({ ok: false, error: 'no run id' });
                    return;
                }
            ...
            'track-sync-members': async (a) => {
                ...
                if (!cfg.repo)
                    return { error: 'No repository configured. Set one in Settings → Integrations → GitHub.' };
                if (!cfg.token)
                    return { error: 'No token. Add one in Settings → Integrations → GitHub.' };
            ...
            'action:set-cli-json': (args) => {
                ...
                const prevToken = fresh.cli?.track?.githubToken;

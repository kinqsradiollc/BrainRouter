...
import {
  ...
  type NoteImageDto,
} from '../lib/notes/imageView.js';

export interface ImageBlockOps {
  /** Store a picked or pasted picture once, and point the block at it. */
  attachImage: (blockId: string, file: { name: string; dataBase64: string }) => Promise<string | null>;
  /** Pull a pasted web address into the store through the shared guarded fetch. */
  storeRemoteImage: (blockId: string, url: string) => Promise<string | null>;
  /** Read a stored picture for rendering. Null when the host could not answer. */
...
export function ImageBlock({ blockId, text, readOnly, ops }: {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void ops.readImage(state.id).then((dto) => {
      if (cancelled) return;
  ...
  const store = useCallback(async (file: File): Promise<void> => {
    ...
    try {
      ...
      const error = await ops.attachImage(blockId, { name: file.name || 'pasted.png', dataBase64: toBase64(buffer) });
          ...
          {readOnly ? null : (
            <button
              ...
              onClick={() => {
                setBusy(true);
                setProblem(null);
                void ops.storeRemoteImage(blockId, state.url)

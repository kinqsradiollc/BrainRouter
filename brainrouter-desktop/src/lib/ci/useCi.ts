...
import { indexPrsByBranch, type PrStatusRow } from './prStatus.js';
...
export interface CiApi {
  pr: PrDetail | null;
  prs: PrListItem[];
  /** §session-pr — all-states PR rows indexed by head branch, for the sidebar's per-session PR icon. */
  prByBranch: Record<string, PrStatusRow>;
  checks: CheckRow[];
  runs: RunRow[];
  loading: boolean;
  ghReady: boolean;
...
export function useCi(opts?: { workspaceRoot?: string | null; onToast?: (msg: string) => void }): CiApi {
  ...
  const [prByBranch, setPrByBranch] = useState<Record<string, PrStatusRow>>({});
  ...
  useEffect(() => {
    const off = window.brainrouter.onEvent((msg) => {
      ...
      else if (op === 'pr' || op === 'prs' || op === 'prmap' || op === 'checks' || op === 'runs') setError(null);
      if (op === 'pr') { setPr((r.pr as PrDetail) ?? null); tick(); }
      else if (op === 'prs') { setPrs((r.prs as PrListItem[]) ?? []); tick(); }
      else if (op === 'prmap') { setPrByBranch(indexPrsByBranch((r.prs as PrStatusRow[]) ?? [])); tick(); }
  ...
  const refresh = (): void => {
    setLoading(true); setError(null); pending.current = 5;
    ...
    window.brainrouter.send({ kind: 'query', id: 'ci:prmap', name: 'git-pr-status-map' });
  ...
  return { pr, prs, prByBranch, checks, runs, loading, ghReady, runDetail, runLog, expandedRunId, watching, error, refresh, expandRun, loadLog, rerunFailed, toggleWatch };
}

...
const tmp = (): string => fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'fsr-')));

test('reads a normal file', () => {
  const ws = tmp(); fs.writeFileSync(path.join(ws, 'a.txt'), 'hello world');
...
test('reading a DIRECTORY returns a typed listing, not a raw EISDIR error', () => {
  const ws = tmp();
  fs.mkdirSync(path.join(ws, 'src')); fs.writeFileSync(path.join(ws, 'src', 'x.ts'), '');
...
test('reading the workspace root (".") lists entries', () => {
  const ws = tmp(); fs.writeFileSync(path.join(ws, 'a.txt'), '');
...
test('workspace file reads keep .asar paths literal and do not descend into archive-looking directories', () => {
  const ws = tmp();
  enableLiteralAsarWorkspacePaths();
  assert.equal((process as NodeJS.Process & { noAsar?: boolean }).noAsar, true);
...
test('listWorkspaceFiles reports truncation at the requested limit', () => {
  const ws = tmp();
  for (let i = 0; i < 4; i++) fs.writeFileSync(path.join(ws, `f${i}.txt`), '');
...
test('isWorkspaceDirectory: true for dirs, false for files / missing / escape', () => {
  const ws = tmp(); fs.mkdirSync(path.join(ws, 'd')); fs.writeFileSync(path.join(ws, 'f.txt'), '');
...
test('reading a file exposes mtimeMs + size for stale-write round-tripping', () => {
  const ws = tmp(); fs.writeFileSync(path.join(ws, 'a.txt'), 'hello');
  const r = readWorkspaceEntry(ws, 'a.txt');
...
test('a binary file (NUL byte) is flagged and returns empty content', () => {
  const ws = tmp(); fs.writeFileSync(path.join(ws, 'bin'), Buffer.from([0x89, 0x50, 0x00, 0x4e, 0x47]));
...
test('statWorkspaceEntry: file / directory / missing / escape', () => {
  const ws = tmp();
  fs.writeFileSync(path.join(ws, 'f.txt'), 'abc'); fs.mkdirSync(path.join(ws, 'd'));
  const f = statWorkspaceEntry(ws, 'f.txt');
  assert.equal(f.exists, true); assert.equal(f.kind, 'file'); assert.equal(f.size, 3); assert.equal(typeof f.mtimeMs, 'number');
...
test('writeWorkspaceEntry: saves a file INSIDE the workspace', () => {
  ...
  assert.equal(r.ok, false); assert.match(r.error ?? '', /parent directory does not exist/);
...
test('writeWorkspaceEntry: stale-write detection via expectedMtimeMs', () => {
  const ws = tmp(); const p = path.join(ws, 'a.txt');
  ...
  const future = opened + 5000; fs.utimesSync(p, new Date(future), new Date(future));
...
test('writeWorkspaceEntry: refuses to overwrite a directory', () => {
  const ws = tmp(); fs.mkdirSync(path.join(ws, 'd'));

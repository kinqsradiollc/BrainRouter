...
import { buildTrayMenuModel, type TrayMenuItem, type TrayAction } from './trayMenu.js';

/** 16×16 template glyph (a small node ring + center dot), black + alpha. */
const TRAY_ICON_B64 =
  '';

export interface TrayHost {
  /** Most-recent-first workspace roots for the submenu. */
  recents: () => string[];
...
function dispatch(action: TrayAction | undefined, host: TrayHost): void {
  if (!action) return;
  switch (action.kind) {
    case 'toggle-window': host.toggleWindow(); break;
    case 'open-workspace': host.openWorkspace(action.root); break;
    case 'quit': host.quit(); break;
  }
}

function toTemplate(items: TrayMenuItem[], host: TrayHost): MenuItemConstructorOptions[] {
 ...
 * either way.
 */
export function setupTray(host: TrayHost): Tray | null {
  try {

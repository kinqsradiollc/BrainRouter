...
export async function pluginConsentSummary(
  ...
  push(p.skills, 'skill'); push(p.personas, 'persona'); push(p.agents, 'agent');
  push(p.orchestrationProfiles, 'orchestration profile'); push(p.hooks, 'hook');
  ...
  return {
    ...
    summary: {
      ...
      provides: {
        ...
        agents: p.agents ?? 0,
        orchestrationProfiles: p.orchestrationProfiles ?? 0,
        commands: 0,
        hooks: p.hooks ?? 0,
        mcpServers: p.mcpServers ?? 0,

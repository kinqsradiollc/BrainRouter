...
|---|---|---|---|
| Next | **0.4.6 → 0.4.7** | Planned — **0.4.6 Codex-style structural refactor** (mirror `codex-rs` module boundaries; decompose the `agent.ts` / `sqlite.ts` / `engine.ts` monoliths into concern modules) then **0.4.7 Codex coding-agent parity** (all 12 gaps from the code-backed comparison: tool-registry, command-segment exec policy, sandbox fail-closed, apply-patch hardening, agent lifecycle, child worktree isolation, typed host protocol) | [`FULL_TASKS.md`](FULL_TASKS.md) §2–§3 |
| Latest | **0.4.5** | Shipped — 2026-06-01 — **release hygiene** (READMEs/deps/policy doc/benchmarks) + **competitive catch-up** (auto code-reindex, browser-verify artifacts, repo-scale metrics, richer headless events) + **CLI polish** (lesson conflict/supersede/staleness, blackboard restore + `/blackboard`, consistent per-user RBAC, transient-LLM retry incl. 5xx/gateway, `/fg` + unified `/ps` + `/stop <id>`, `/agents create` validation + `--dry-run`, width-aware tables, offload footer segment, composer history + flag-suggestions + tab-to-end cursor) | [`brainrouter-changelog/0.4.5.md`](brainrouter-changelog/0.4.5.md) |

...
|---|---|---|---|
| Now | **0.4.9** | In progress — styling approved. **Dashboard redesign** to BrainRouter's own design language ("The Memory Instrument" — retire the slash/Inter/serif/golden copycat; one Signal accent `#34C28E` + Recall-Heat data ramp; Geist + Geist Mono; color-stepped elevation) shipped to the branch: whole-app token restyle, an ecosystem section featuring MCP + CLI + dashboard, full de-gold/de-glow polish, and `prefers-reduced-motion` + keyboard-focus a11y. **API hardening** shipped: fail-closed JWT secret + `/me` key-leak fix, security headers + strict CORS allowlist, reusable zod validation middleware, dual rate-limiting + a terminal error envelope, and a security audit (server + packages vuln-free; `SECURITY.md`). | [`docs/design/themes/brainrouter.md`](docs/design/themes/brainrouter.md) |
...
| **[0.4.9](docs/design/themes/brainrouter.md)** | **Dashboard redesign** to BrainRouter's own design language ("The Memory Instrument" — retire the slash/Inter/serif/golden copycat; Signal accent + Recall-Heat ramp; Geist + mono; color-stepped elevation; ecosystem section; de-gold/de-glow; reduced-motion + focus a11y) + **API hardening** (fail-closed JWT + `/me` leak fix, security headers + strict CORS, reusable zod validation, dual rate-limiting + terminal error envelope, security audit + `SECURITY.md`, vuln-free, behavior-preserving) | In progress — styling approved |

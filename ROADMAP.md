...
| **[0.3.7](brainrouter-roadmap/0.3.7.md)** | **Terminal UI redesign + in-terminal config wizard + full Ink chat REPL + CLI/server env separation + multi-agent registry foundations** | _In-flight_ — Item 6 shipped + scope expanded (wizard + `/config` + `/login` + Ink chat REPL as unconditional default + CLI reads no `.env` + MCP child stderr piped + wizard Skip fix); **Item 7** (multi-agent Phase 1 — data-driven agent registry + tier/depth caps) added as a small additive item; Items 1–5 (cron `/schedule`, `/release-notes`, hooks doc, deer-flow Strict Tool-Call Recovery, per-vendor MCP install snippets) cut and carried into 0.3.8 |
| [0.4.0](brainrouter-roadmap/0.4.0.md) | **Federation — many agents, one memory + multi-agent Phase 2 (typed delegation, parent context, route_task, output contracts)** | Designed — 5 federation stages + memory-quality augmentations from deer-flow/semble + CLI multi-agent Phase 2 (the federation handoff packet IS the parent context snapshot) |
| [0.4.x](brainrouter-roadmap/0.4.x.md) | Post-federation polish + multi-agent Phases 3–6 | Planned — dynamic subagents, worktree isolation, `/rewind`, `/context per-skill`, benchmark harness, progressive skill loading, code-aware chunking, **plus** ownership contracts, tool budgeting, supervisor gates, review fan-out, worker threads, packs, agent debugger, brain-agent awareness |

...
|---|---|---|---|
| Next | **0.4.7** | Planned — **Codex coding-agent parity** (all 12 gaps from the code-backed comparison: tool-registry, command-segment exec policy, sandbox fail-closed, apply-patch hardening, agent lifecycle, child worktree isolation, typed host protocol) | [`FULL_TASKS.md`](FULL_TASKS.md) §3 |
| Latest | **0.4.6** | Shipped — 2026-06-01 — **structural refactor** (behavior-preserving modularization of the engine / CLI / dashboard monoliths into concern-scoped modules: `runtime/exec/`, `agent/applyPatch`, `agent/tools`, engine domain ops, ChatApp components, dashboard demo hooks; 14 PRs, full suite green, zero behavior change) | [`brainrouter-changelog/0.4.6.md`](brainrouter-changelog/0.4.6.md) |

...
| /diagram <kind> <what…> \| draft [--layers a,b] [--prefix p] \| validate <file> \| render <file> [--slug s] [--theme t] [--no-verify] \| diff <slug> [--base rev] \| list \| show <slug> \| open <slug> | Typed, validated system maps (architecture, workflow, sequence, dataflow, lifecycle) rendered to self-contained HTML with a receipt under .brainrouter/diagrams/ |
| /design <verb> [targets…] [--mode persuade\|operate\|read\|experience] [--world id] \| detect [paths…] [--rules a,b] [--json] \| rules \| hooks [on\|off\|immediate\|full] \| verbs | Design as verbs (critique, audit, polish, harden, typeset, layout, colorize, document, product, …) routed to the one design skill, plus the deterministic detector: slop tells, quality defects, design.md token drift; suppressions in .brainrouter/design-detector.json |

...
| `packages/core` | Headless domains with curated package exports and a minimal root barrel; `terminal/` owns pure geometry, cursor, and reattachment policy | Several domains mix contracts/policy/adapters; background shell execution and execution-host spawning still mix process/filesystem effects with Core runtime ownership; large extension and orchestration modules remain | Migrate one domain at a time to contracts/domain/policy/ports/services/adapters as earned; preserve curated subsystem entrypoints and keep native PTY/process effects in hosts |

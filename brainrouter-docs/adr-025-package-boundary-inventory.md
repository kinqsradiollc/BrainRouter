...
| `packages/core` | Headless domains with curated package exports and a minimal root barrel; terminal, background-shell, and durable-task policy now call explicit privileged hosts | Several domains mix contracts/policy/adapters; execution-host spawning still imports process and environment effects directly; large extension and orchestration modules remain | Migrate one domain at a time to contracts/domain/policy/ports/services/adapters as earned; preserve curated subsystem entrypoints and keep native PTY, persistence, identity, clock, process, and filesystem effects behind explicit host ports |

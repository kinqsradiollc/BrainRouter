...
| `packages/core` | Headless domains with curated package exports and a minimal root barrel; `terminal/` owns pure session policy and background-shell registry/polling policy calls one private privileged host adapter | Several domains mix contracts/policy/adapters; durable background-task persistence and execution-host spawning still import identity, filesystem, process, or storage owners directly; large extension and orchestration modules remain | Migrate one domain at a time to contracts/domain/policy/ports/services/adapters as earned; preserve curated subsystem entrypoints and keep native PTY/process/filesystem effects behind explicit host ports |

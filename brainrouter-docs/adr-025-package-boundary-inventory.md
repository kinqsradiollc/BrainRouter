...
| `packages/agent-protocol` | Zero-dependency agent-host vocabulary with structural guards; the root curates private agent lifecycle, interaction, delegation, assurance, and terminal-session modules | Lifecycle and authority records remain structural mirrors that need parity fixtures before deeper agent-runtime extraction | Keep the root entrypoint unchanged and all guards pure; evolve host-neutral lifecycle and terminal wire records here only when Core and host fixtures prove parity |
| `packages/core` | Headless domains with curated package exports and a minimal root barrel; terminal, background, and connector checkpoint policy now call explicit privileged hosts/adapters | Several domains mix contracts/policy/adapters; execution-host spawning still imports process and environment effects directly; large extension and orchestration modules remain | Migrate one domain at a time to contracts/domain/policy/ports/services/adapters as earned; preserve curated subsystem entrypoints and keep native PTY, credentials, cwd, network, persistence, identity, clock, process, and filesystem effects behind named ports/adapters |
...
| Done | `packages/agent-protocol/src/index.ts` | Curated root | Events, commands, interaction, callback projection, envelope stamping, assurance, and terminal-session vocabulary have private sibling owners | Preserve the root-only public entrypoint and dependency-free package contract |
| Done | Core/Desktop terminal session boundary | Shared geometry, cursor, reattachment, shell-catalog, open, and read records | Core owns deterministic session normalization; Electron owns executable discovery, environment access, native PTY allocation, and process lifecycle | Keep renderer input/output validation on the shared protocol and keep privileged executable/process capabilities out of Core policy |
| Done | Core background-shell execution | In-process registry, bounded output polling, and cancellation policy previously imported process, filesystem, random identity, and state-path owners | Policy calls one private host contract; the Node adapter owns detached spawning, logs, process-tree signals, UUIDs, clocks, and host-exit registration | Preserve approval-before-start, workspace log scope, incremental byte offsets, UTF-8 boundaries, whole-tree cancellation, and cleanup fixtures |
| Done | Core durable background-task store and reconciliation | Workspace/session task policy previously imported state-file I/O, UUID, clock, owner PID, and process liveness directly | Policy retains the unchanged public store/service API while one private Node host owns persistence, identity, time, owner tagging, and liveness probes | Preserve durable reopen, filters, transition timestamps, progress, memory linkage, orphan reconciliation, terminal retention, and workspace/session scope |
| Done | Core connector checkpoint composition | Runtime dispatch already separates per-source network/filesystem clients and connector stores, but default environment-token and cwd resolution lived in policy | One host-neutral runtime contract supplies environment and cwd; the Node implementation remains the default adapter while hosts may inject their own context | Preserve explicit credential modes, sealed OAuth precedence, relative-root resolution, per-source dispatch, workspace-scoped persistence, failure receipts, and CLI/Desktop parity |
| Done | Core storage service boundary | Workspace/session-scoped checkpoint and file-mutation persistence plus deterministic restore planning | Core contracts and restore policy are host-neutral; a persistence port owns the durable adapter | Keep legacy storage imports compatible while services depend on the port and pure policy never imports filesystem owners |
| Done | Core execution-host boundary | Local, WSL, and SSH command planning plus synchronous host execution | Core keeps validation, argv construction, and quoting deterministic; a Node adapter owns process creation and environment access | Preserve the existing execution entrypoint while allowing hosts and tests to inject an executor |
| Done | Core passive worktree awareness | Porcelain parsing and bounded sibling-worktree projection previously launched Git directly | Policy parses and caps host output; a private Node adapter owns Git process creation and timeout | Preserve exact self-exclusion, branch/detached labels, 12-entry cap, and best-effort failure behavior |
...
| P0 | `brainrouter/src/integrations/prSecurityReview.ts` | 800 lines | Checkout/diff intake, model review, GitHub comments/checks, coverage, and compatibility aliases share one integration; deterministic finding lifecycle now delegates to Core through a compatibility façade | Adapt to Core assurance validation/gate policy, then extract the assurance service, GitHub port/adapter, packet builder, and publication adapter behind current functions |

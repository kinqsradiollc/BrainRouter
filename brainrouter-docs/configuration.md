...
| `BRAINROUTER_RECALL_RERANK_BLEND_ALPHA` | `0.6` | MEM-BLEND (0.4.14): weight of the cross-encoder relevance vs the pre-rerank score (RRF + half-life recency) when combining them, by **reciprocal rank** (cross-encoder scores are bimodal, so a raw-score blend is a no-op). `1` = pure reranker (legacy — replaces the order); `0` = pure retriever order. Keeps recency in play (a recency baseline can't beat the pipeline) and stops low-similarity reflective gold from being demoted. Clamp [0,1]. |

...
| `BRAINROUTER_RERANKER_TIMEOUT_MS` | `25000` | Per-call reranker timeout, clamp [1000, 120000]. Bounded — never inherits the generative local floor; on timeout recall falls back to RRF fast. Raised 15000→25000 (SPEC 01): a CPU cross-encoder under multi-agent load (N separate processes hit one single-worker server) can legitimately take 15-25s, and the old 15s aborted those mid-flight → RRF every trip. Lower to ~10000-15000 on a GPU/vLLM or Cohere backend once p95 is measured. |
| `BRAINROUTER_RERANKER_MAX_CONCURRENT` | `1` | Concurrency cap on outbound rerank requests from this process (separate pool from the generative + embedding caps). Default 1 — a local CPU cross-encoder is single-worker, so concurrent calls just queue-stack it into timeouts. Raise only on a GPU/vLLM backend. |
| `BRAINROUTER_RERANKER_ACQUIRE_WAIT_MS` | `15000` | SPEC 01: max time a recall waits for the (cap-1) reranker slot before shedding to RRF, clamp [1000, 120000]. Under parallel load the slot is contended; rather than queue unboundedly (stalling the `memory_recall` reply → "client disconnected"), a recall that can't get the slot in this window skips the cross-encoder. This is load-shedding, NOT a failure — it does NOT trip the breaker. The responsiveness↔quality knob; the real cure for CPU saturation is a GPU backend. |

...
- **ADR-041 (plug-and-play runtime):** two safe, behaviour-neutral foundations shipped to `release/0.4.21` — **A41-6** (`IMemoryStoreComposite`, removed 15 `as unknown as *Store` casts) and **D1 foundation** (#1418: the `ProviderRegistry` class replacing the `ReadonlyMap`, with `register`/`replace`/`dispose` + `hasBuiltin` — identical reads, the seam ADR-043 S2's `ProviderDialer` attaches to). **D1 is now complete** (#1418 foundation + #1423 extension-unify — extension providers are live to routing). **D3 (capability ports) is underway: the `FilesystemPort` is COMPLETE** (#1428/#1429/#1431) — all five file tools (`write_file`/`read_file`/`list_dir`/`edit_file`/`notebook_edit`) run through an injectable, swappable async port (`agent/fs/filesystemPort.ts`), defaulted to `nodeFilesystemPort` = byte-identical `node:fs`. Remaining: D3's `ShellPort` (the security-critical `run_command` exec) + `SubprocessPort` (`spawnWorkerThread`), then D4 (`IAgent` + phase hooks), D8 (guarded tool pipeline), D10 (execution worlds) — the hot-path refactors. Seams in place: `ProviderRegistry`, `EdgeDialer`, `FilesystemPort`.

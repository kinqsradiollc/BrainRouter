...
| A25-4c5 | `[ ]` | Extract permissioned tool-call repair, dispatch, result pairing, and cancellation into a tool-execution phase | Core fixtures preserve model-visible schemas, authority ceilings, approval order, parallel-safe batches, malformed-call recovery, exact result order, and interruption behavior |
...
| A25-4c5b | `[ ]` | Extract permission, required-skill, extension, and approval gates plus local/MCP/orchestration dispatch behind one typed executor | Tool fixtures preserve fail-closed authority, exact callback order, profile/skill ceilings, and error projection |
...
| A25-4c5b2 | `[ ]` | Extract local, orchestration, and MCP adapter invocation plus result/error projection | Dispatch fixtures preserve active-turn lifecycle, approval timing, skill loading, extension adaptation, summaries, and denial classification |
| A25-4c5c | `[ ]` | Extract parallel-safe batch scheduling, ordered result publication, interruption fill, and orphan repair | Batch fixtures preserve concurrency limits, original result order, cancellation, and complete tool-call pairing |
...
| A25-6b2 | `[ ]` | Extract permission and dialog prompting behind a bounded manager | Permission fixtures preserve saved decisions, grants, timeout/cancel behavior, selected-tab presentation, and event order |
| A25-6b3 | `[ ]` | Extract download ownership and lifecycle behind a workspace-scoped manager | Download fixtures preserve workspace scope, agent gesture authority, progress events, cancellation, path selection, and listener cleanup |

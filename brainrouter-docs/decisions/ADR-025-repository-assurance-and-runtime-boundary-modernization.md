...
| A25-13c2b2 | `[ ]` | Bind the manual API and scheduler to single-attempt deep execution without changing ordinary diff or assessment paths | Route, scheduler, and executor fixtures prove server-built policy, request-source/scope validation, model/time ceilings, cancellation, and no webhook/implicit activation |

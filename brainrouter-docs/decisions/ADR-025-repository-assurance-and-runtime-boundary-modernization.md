...
| A25-5d2 | `[ ]` | Extract manifest normalization, profile-default resolution, compatibility translation, and safe serialization into a pure policy module | Policy fixtures preserve no-manifest-safe defaults, profile precedence, explicit-catalog selection, secret/path redaction, forward-compatible extras, diagnostics, and byte bounds |

...
| A25-2 | `[x]` | Define shared-contract placement and package-boundary guard pilot | A25-1 | `scripts/check-package-boundaries.mjs` enforces the graph during lint and staged-source checks; negative fixtures cover leaves, application back-edges, browser safety, Dashboard isolation, curated Core exports, and compiled-path rejection in every maintained consumer |
...
| A25-5 | `[x]` | Modernize workspace/profile/skills/capability and onboarding boundaries | A25-2 | Compiled manifest, catalog, task-capability, tool-profile, memory-capture, migration, claim, pair-recovery, and no-manifest fixtures preserve reviewed selection and exact legacy compatibility |
| A25-6 | `[x]` | Modernize browser, terminal, background, connectors, storage, and worktree boundaries in domain slices | A25-2 | Compiled per-domain host-boundary fixtures preserve ownership, permission, cancellation, persistence, and cleanup; a direct policy/service import audit finds no privileged process or filesystem owners |
...
| A25-14 | `[x]` | Update contributor docs, ADR/planning/code-quality skills, and profile/task activation policy for the modernized ownership model | A25-2 through A25-6 | Contributor and topical rules keep global invariants concise; four focused skills own planning, ADR, review, and cleanup procedure; Core builds and nine planning/required-skill fixtures cover model-neutral plan-only, routine engineering, security-boundary, research, writing, data-science, custom-schema, active-goal, disabled-skill, and no-manifest activation |
| A25-15 | `[x]` | Remove obsolete internal paths only after consumers leave compatibility façades | A25-3 through A25-14 | Maintained renderer consumers use 14 focused browser-safe Core exports; the extension API spec uses the curated extension export; the `./dist/*` wildcard and lint exception are removed; boundary fixtures reject compiled paths in both host and renderer code; the repository import graph has no supported Core deep import |
...
| A25-6c | `[x]` | Separate terminal, background, connector, storage, and worktree policy/services from privileged host adapters | Compiled worktree, terminal, background-shell, durable-task, connector, storage, worktree-awareness, and execution structural fixtures pass; a direct import audit keeps privileged effects in the named adapters |
...
| A25-6c4 | `[x]` | Separate background-shell registry, polling, and cancellation policy from detached process, log-file, identity, clock, signal, and host-exit effects | Core build plus 19 background execution, shell-policy, end-to-end tool, and structural-boundary fixtures preserve approval-before-start, workspace-scoped logs, byte-exact polling, UTF-8 completion, whole-process-tree cancellation, terminal status, and cleanup |
| A25-6c5 | `[x]` | Separate durable background-task transition, filtering, retention, and reconciliation policy from persistence, identity, clock, owner-PID, and liveness effects | Core build plus 21 store, service, dashboard, reconciliation, and structural-boundary fixtures preserve workspace/session scope, durable reopen, timestamps, progress, memory links, filtering, orphan failure, retention, and public imports |
| A25-6c6 | `[x]` | Separate connector checkpoint policy from process environment and current-directory capabilities while retaining explicit source and persistence adapters | Core build plus 33 checkpoint, credential, relative-root, source, persistence, failure, and structural-boundary fixtures preserve static/OAuth precedence, workspace scope, dispatch, filesystem bounds, checkpoint receipts, and host parity |
| A25-6c7 | `[x]` | Separate storage contracts, deterministic restore policy, and the workspace-scoped service from durable checkpoint and file-mutation adapters | Types, protocol, and Core builds plus 24 focused checkpoint, queue, file-snapshot, rewind, service, and structural fixtures preserve public imports, bounded persistence, workspace/session scoping, recovery behavior, and deterministic restore output |
| A25-6d | `[x]` | Separate local, WSL, and SSH command planning from privileged process creation and environment access behind the unchanged execution-host entrypoint | Types, protocol, and Core builds plus six focused planning, injection, validation, and structural fixtures preserve exact argv, quoting, timeout, cwd, result, and failure behavior |
| A25-6c8 | `[x]` | Separate passive sibling-worktree parsing and bounded prompt projection from privileged Git process execution | Types, protocol, and Core builds plus five parsing, prompt, injected-host, best-effort-failure, and structural fixtures preserve self-exclusion, labels, bounded output, timeout ownership, and no-error fallback |

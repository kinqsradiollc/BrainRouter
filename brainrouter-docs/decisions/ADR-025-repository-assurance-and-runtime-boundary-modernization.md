...
| A25-5d3 | `[ ]` | Extract trusted manifest load/save and transaction recovery composition into a filesystem store | Store fixtures preserve fixed paths, bounded/no-follow reads, safe writes, recovery order, diagnostic recording, absent/corrupt compatibility, and the public facade |

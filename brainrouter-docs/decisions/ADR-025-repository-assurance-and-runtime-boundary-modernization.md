...
| A25-13c1 | `[x]` | Define the dependency-free deep-review policy, accepted telemetry threshold receipt, and activation decision | Types/Core fixtures pin tenant/repository/program scope, explicit-manual-only activation, program-specific thresholds, cost/time/model/tool ceilings, malformed-telemetry rejection, fail-closed cancellation, bounded coverage labeling, tamper detection, and diff-review non-escalation |

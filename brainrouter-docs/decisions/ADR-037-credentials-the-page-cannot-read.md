...
**Status:** ACCEPTED — backend security IN PRODUCTION on `release/0.4.21` (2026-08-17); dashboard cookie cutover in progress. Owner approved the topology-agnostic design (`SameSite=None` + full CSRF, correct under any domain shape). SHIPPED: **B1** the revocable refresh-session store (migration 065 `refresh_sessions`; issue/rotate/reuse-revokes-chain wired into signin/refresh/signout + password-reset; `/refresh` adopts legacy tokens so no forced signout on deploy) — the ADR's premise that this "interim mitigation is already merged" was FALSE, it was dead code (30-day un-revocable token); **B2** boot-time refusal of `*`+credentials on both API + audio-gateway ports (D3); **B3** the `br_refresh` httpOnly;Secure;SameSite=None cookie + `csrfOriginGuard` (Origin check, bearer/no-cookie pass-through) + double-submit CSRF token, all accepted server-side alongside body+bearer (D1/D2); **D-1** the planner takes identity from `/api/auth/me` instead of decoding the JWT. REMAINING (needs the running stack for §5's read-the-session-from-console acceptance test): **D-2** flip dashboard transport to the cookie + in-memory access token + D5 forced-signout + a WebSocket connect-ticket; **D-3** delete the localStorage accessors + show-once the API key; **B4** stop emitting body tokens (cookie sole transport). DESIGN NOTE surfaced during D-2: B3's fetched-into-memory CSRF token cannot bootstrap on a page reload (memory is empty, but the cookie-path `/refresh` requires the token) — the cutover should revise it to a **readable `br_csrf` double-submit cookie** (survives reload, still unreadable cross-origin).

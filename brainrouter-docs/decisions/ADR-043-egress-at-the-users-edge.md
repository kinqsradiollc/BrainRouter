...
**Status:** Accepted — **implemented** (2026-08-16→20, `release/0.4.21`). The live edge egress tunnel is complete end-to-end: server data path, desktop client edge, gateway activation, per-org consent + kill-switch + telemetry, the client indicator, and the §7 acceptance tests; `openai`/`anthropic` declare the `clientTunnel` capability so an opted-in org routes their traffic through its own devices. The only unbuilt item is the S5 vended-token *mode*, which is provider-gated (no chat API vends scoped session tokens today; the `egressCapabilities.vendableToken` flag awaits one). Historical build detail follows. — S1/S1b + S2 implemented and S3/S4a built (2026-08-17→19, `release/0.4.21`); ADR-041 (the seam these depended on) has landed, so S3–S5 are no longer gated. **S1 (D5): the gateway rate-shaper** — per-upstream-key concurrency + rpm token-bucket + Retry-After backoff + bounded fair-share queue (`services/gateway/rateShaper.ts`, keyed for per-org sharding), wired into `chatRoutes` fail-open: an upstream 429's Retry-After parks the key so a burst stops hammering a provider that already refused (the review-bot free-tier wedge), and requests to a parked key fast-fail with the hint. S1b wires the concurrency cap + rpm budget proactively: each dial reserves a per-key slot (released in `finally`), so a burst is bounded BEFORE it stampedes the upstream, not only reactively after a 429 parks the key. Budgets are injectable via `GatewayDataPlaneOptions.rateShaper` (generous defaults 64 concurrent / 600 rpm / 256 queued; fail-open). **S2 (edge-dialer seam) SHIPPED**: the `EdgeDialer` type + `directDialer` default are wired into `fetchUpstreamWithPolicy` / `chatRoutes` (`options.upstream.dispatcherFactory ?? directDialer`), byte-identical to server egress today. **S3 + S4a are BUILT and unit-tested but ship DARK (unwired)**: `tunnelDialer` / `framedTransport` / `egressTicket` / `tunnelTransport` (S3) and `selectEdgeDialer`'s three-leg opt-in gate + the connect-time fallback ladder (S4a) all exist, but `selectEdgeDialer` is not yet called by the dial path and the relay-backed `EgressChannelOpener` (the live tunnel over `remoteRelay`'s ticketed splice) is not yet built — so at runtime every request still uses `directDialer`. The **ADR-041 dependency is SATISFIED**: `ProviderDefinition.egressCapabilities` / `egressMode` are in `packages/core` today (the `ProviderDialer` name in §6 is vestigial; the seam shipped as gateway-local `EdgeDialer`). **S3b–S4 (the LIVE tunnel) SHIPPED (2026-08-19→20, `release/0.4.21`):** the full data path (C1 relay control-plane adapter → C2 `/egress-relay` endpoint → C3 relay-backed `EgressChannelOpener` → C4 device→server `/egress-control` channel + concrete device-session authenticator + edge push bridge), the desktop client edge (C5a control client + C5b SSRF-guarded dial/splice handler + C5-wire host DI), the activation (C6a `EgressTunnelService` gateway-boot composition + C6b route selection: `selectUpstreamForRequest` picks the edge dialer only when the provider declares `clientTunnel`, the org opted in, and a device is online — byte-neutral behind `GATEWAY_EGRESS_ENABLED`), and C7's mechanical operator surface (per-org consent `/api/admin/egress-settings` + global kill-switch + per-request `egress_mode` telemetry). Every security-sensitive component was adversarially reviewed; the §7 acceptance criteria run as tests (`egressAcceptance.test.ts`: traffic egresses through the device to a real target, tampered/refused dial fails closed, one-user isolation). **Activated (2026-08-20):** the two primary cloud providers (`openai`, `anthropic`) declare `egressCapabilities.clientTunnel` + `egressMode: 'auto'`, so an org that opts in (default off) routes their traffic to those providers through its own devices; extending to the other cloud vendors is a one-line-per-provider addition. **Indicator SHIPPED (2026-08-20):** the desktop window chrome shows a subtle "traffic leaves from this device" status dot whenever this device is actively holding an egress control channel (`EgressTunnelBadge` polling the `egress-tunnel-status` host query). **Only remaining (provider-gated):** the vendable-token vend endpoint + client-direct path (S5/D3) — a distinct egress *mode* that cannot be built until a provider offers scoped session tokens (§2 option B: "adopted where offered"); the `egressCapabilities.vendableToken` flag is in place for when one does.

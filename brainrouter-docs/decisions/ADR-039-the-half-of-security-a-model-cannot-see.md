...
**Status:** Accepted — scoped **this-repo-only** (2026-08-17). The D8 blocker (this-repo-only vs customer-facing) is resolved to the conservative scope: BrainRouter runs the flow/taint analysis on its OWN repository as a review input, with **no customer-facing offering** — so there is no third-party licensing exposure and the build is unblocked. (Expanding to a customer-facing product later would reopen D8 and need a licensing review; that is explicitly out of scope now.) Implementation is a phased program (flow-analysis engine against the exact-SHA checkout with its own DB-build stage, an owned barrier/source/sink model pack, and integration into the review pipeline) —  sized as its own track, not a single slice. **Down payment (0.4.21):** the concrete vulnerabilities this ADR cites as evidence were located + adversarially verified and fixed ahead of the engine — the ~23s quadratic-regex ReDoS on the recall/briefing path (#1413) and the LM Studio model-probe SSRF that was the unguarded "fourth path" (#1414). The three runtime provider-endpoint SSRFs (embeddings / rerank / memory-pipeline chat) are also fixed (#1416): a validate-then-fetch guard through `upstreamProbePolicy` refuses an internal target before dialing (self-hosted local backends opt in via `BRAINROUTER_UPSTREAM_ALLOWLIST`, documented in deploy/brain/README; a narrow DNS-rebinding TOCTOU is a noted follow-up). All six confirmed concrete vulnerabilities are now closed. **The flow/taint engine SHIPPED as a CodeQL integration, not a bespoke build** — per D3 ("integrate an off-the-shelf DB-first scanner, do not invent our own"). What is implemented and tested, and wired end-to-end into the review path: S2 (the SARIF→source→sink candidate pipeline, `release/0.4.21`) — `prSecurityReview` supplies the CodeQL taint-path provider for `refs/pull/N/head`, `scheduledPrReview` forwards it through `assuranceReady`, and `repositoryContextComposition` wraps the base impact assembler with `CodeqlAugmentedImpactAssembler` so CodeQL's source→sink paths join the review candidates on the same footing (GitHub-only; the repo's public-repo CodeQL default-setup run supplies the SARIF, no GHAS needed); the §6 replay corpus (`release/0.4.21`); **D6 source→hops→sink paths** and **D5 "not analyzed" honesty** — an absent scan surfaces a `CODEQL_NOT_ANALYZED` coverage limitation rather than reading as clean (`release/0.4.22`); and **the D4 barrier pack** — OUR ten chokepoints as versioned data with a parity check that fails the build on a renamed/removed guard (`release/0.4.22`, `adr039BarrierPack.ts`). Two things remain **owner-activated**, gated on one repository-settings change (the repo runs CodeQL *default* setup, which cannot coexist with an advanced workflow): the `security-extended` query suite, and rendering the D4 barrier pack into the CodeQL barrier model so the engine *proves* a barrier dominates a tainted path and drops the fixed finding — the sound placement for barriers (a textual "the guard symbol appears, so dispute it" rule in the model verifier would suppress exactly the modelProbe.ts three-of-four-paths true positive). See `brainrouter-docs/setup/adr039-codeql-advanced-setup.md`. The bespoke from-scratch engine described in the body is therefore **superseded** by this integration for the accepted this-repo scope.

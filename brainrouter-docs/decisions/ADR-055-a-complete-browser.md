...
**Status:** ACCEPTED — phased implementation on `release/0.4.22`. **Eleven of thirteen rows shipped.** Agent side: P1 images the model can see, P2 coordinate clicks (credential-safe), P3 snapshot scope + shadow DOM, P4 `page.find` locator, P5 action receipts, P6 two-way human-needed hand-back, P8 downloads inbox, P11 the coherent restricted seat + human-only certificates. Human side: P7 share-a-tab, P9a bookmarks/history/omnibox engine, P10 PDF viewer + HTML5 fullscreen + print + real per-site permissions + context-menu parity, and the P9b/P10b/P7b chrome UI (omnibox autocomplete, bookmark star + ⌘D, Bookmarks/History drawers, site-info popover, Save-as-PDF, per-tab share toggle, fullscreen chrome). **Remaining:** P12 the agent-as-human harness battery (needs a real packaged run to write honestly) and P13 the optional pop-out window, plus three niceties — the new-tab page, tab search (⌘⇧A), and pinned tabs. A post-merge adversarial self-review of the agent-side rows confirmed and fixed one real defect (P2's credential refusal over-refused ordinary buttons). · **Builds on:** ADR-024 (the browser action loop and lanes — §5

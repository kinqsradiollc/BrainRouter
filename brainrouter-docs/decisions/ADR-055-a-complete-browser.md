...
**Status:** ACCEPTED — phased implementation on `release/0.4.22`. **Eight rows shipped** (the whole agent-control surface): P1 images the model can see, P2 coordinate clicks with credential refusal, P3 snapshot scope + shadow DOM, P4 `page.find` locator, P5 action receipts, P6 two-way human-needed hand-back, P8 downloads inbox, P11 the coherent restricted seat + human-only certificates. **Remaining (need the owner's live desktop, not buildable headless):** P7 share-a-tab and P9/P10/P13 the human chrome (bookmarks, history, omnibox autocomplete, site-info, PDF, fullscreen, print, pop-out) are renderer work that must be built screenshot-driven with the owner watching; P12 the agent-as-human harness battery needs a real packaged run; the explicit hand-back button / takeover banner (P6) and `page.selection` + clipboard (P8) are small renderer/agent follow-ups. · **Builds on:** ADR-024 (the browser action loop and lanes — §5

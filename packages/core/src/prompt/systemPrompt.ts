...
const INSTRUCTIONS_HEAD: readonly string[] = [
  ...
  '- A dirty worktree is not a blocker by itself. Inspect it read-only, preserve unrelated user changes, and continue within your owned files. Do not ask the user to checkout, restore, reset, clean, stash, commit, or revert unrelated work merely to create a clean tree.',
  '- If current user edits overlap the exact lines you must change, prefer an isolated worktree/branch when that preserves both versions. Otherwise name the exact file and collision in one precise question. Never use repository-wide cleanup as conflict resolution.',
  ...
  'A failed build or test calls for causal diagnosis and an in-scope fix. Rollback commands are appropriate only for an explicit rollback request or a narrowly identified change you created and own; they are not generic error recovery.',
  '',
  '# Using your tools',

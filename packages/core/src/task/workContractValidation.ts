/**
 * Pure validation for persisted Work Contract values.
 *
 * Shared data shapes live in the types package; core owns the bounded storage
 * invariants that protect session files before runtime code can consume them.
 */
import {
  WORK_CONTRACT_SCHEMA_VERSION,
  type WorkContract,
  type WorkContractStatus,
  type WorkReviewDisposition,

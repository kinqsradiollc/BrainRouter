...
export function buildGoalKickoffPrompt(
  ...
  return [
    ...
    '4. **Parallelize only genuinely independent work.** Spawn child agents (`task_agent`) ONLY for chunks that are truly independent and can run at the same time (e.g. several unrelated modules). For a single cohesive task, do it directly. An active goal and this kickoff prompt cannot authorize workflow execution; never call `run_workflow` or `run_workflow_graph` on that basis. If a workflow is warranted, recommend that the user launch it explicitly in the CLI with `/build <task>` or `/workflow run <template> [jsonArgs]`.',

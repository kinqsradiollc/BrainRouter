...
export function setCurrentWorkflow(workspaceRoot: string, slug: string, sessionKey?: string): void {
  ...
  writeJsonFile(getStateFile(workspaceRoot, CURRENT_POINTER_FILE), { slug, at: ts });
  if (sessionKey) {
...
export function getCurrentWorkflow(workspaceRoot: string, sessionKey?: string): string | undefined {
  ...
  const ptr = readJsonFile<{ slug?: string } | null>(getStateFile(workspaceRoot, CURRENT_POINTER_FILE), null);
  return ptr?.slug;
}

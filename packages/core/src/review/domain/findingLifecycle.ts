/**
 * Deterministic finding identity and lifecycle reconciliation.
 *
 * Extracted from the backend without behavior change so every host uses one
 * line-movement-tolerant fingerprint and one complete/partial reconciliation
 * rule. This module is pure and adapter-free.
 */
...
function normalizeFinding(
  lens: string,
  finding: LifecycleFindingInput,
): NormalizedLifecycleFinding {
  const file = String(finding.file ?? '').trim().replace(/^\.\//, '').replace(/\\/g, '/');
  const title = String(finding.title ?? '').trim();
  const line = Number.isInteger(finding.line) && Number(finding.line) > 0
    ? Number(finding.line)
    : undefined;
  const endLine = Number.isInteger(finding.endLine) && Number(finding.endLine) >= (line ?? 1)
...
export function reconcileFindingLifecycle(
  input: ReconcileFindingLifecycleInput,
  ...
  for (const raw of input.incoming ?? []) {
    ...
    const exact = (byFingerprint.get(finding.fingerprint) ?? [])

...
function checkString(target: Record<string, unknown>, key: string, path: string, issues: string[]): void {
...
function checkForbiddenSecretKeys(
  value: unknown,
  issues: string[],
  path = 'run',
  ...
  if (Array.isArray(value)) {
    value.forEach((item, index) => checkForbiddenSecretKeys(item, issues, `${path}[${index}]`, seen, depth + 1));
    return;
  }

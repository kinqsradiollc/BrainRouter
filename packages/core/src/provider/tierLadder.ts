...
/**
 * Built-in/user ladders come from providers.json, not from provider modules.
 * Provider definitions deliberately avoid model ids; the ladder is configuration
 * data because it necessarily names models and changes independently from the
 * wire contract.
 */
function buildDefaultLaddersFromConfig(): Record<string, TierLadder> {
  ...
  for (const [providerId, entry] of Object.entries(cfg.providers)) {
    ...
    out[providerId] = { provider: providerId, ladder: { flash: entry.tierLadder.flash, standard: entry.tierLadder.standard, pro: entry.tierLadder.pro } };
  }
  return out;
}

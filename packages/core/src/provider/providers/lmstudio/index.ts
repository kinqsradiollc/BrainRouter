 ...
 *  imports `config`, so a static import here would form an import cycle the
 *  moment `config.ts` imports the provider registry (config → providers →
...
/** LM Studio — a LOCAL OpenAI-compatible server. A blank API key is fine. Its
 *  native `/api/v1/models` endpoint (richer than `/v1/models`) is fetched and
 *  cached by the helpers below, keyed off this endpoint. */
export const lmstudio: ProviderDefinition = {
  ...
  // `reasoning: { effort }` object; its support for the flat `reasoning_effort`
  // field is unconfirmed and it accepts-and-ignores unknown params — so we send
  // BOTH shapes to work across LM Studio versions. Inherits the default 'any'
  // model gate: it accepts-and-ignores the field for a non-reasoning model, so we
  // don't need a name allowlist here (unlike OpenAI).
...
// ---------------------------------------------------------------------------
// LM Studio native models API (0.3.9 follow-up; merged into this module 0.4.15
// so there is exactly ONE LM Studio file — the provider definition above and
// the native-endpoint enrichment below live together).
//
...
//          accepts — informs effort plumbing)
//        - `type` ("llm" vs "embedding" — we filter embeddings out
//          since they're not chat models)
//
// These helpers fetch the native endpoint, cache the result per session, and
// expose a synchronous lookup so the rest of the CLI (`contextWindowFor`,
// `/status`, the model picker) can consult it without adding latency to render
// paths.
//
// When the active endpoint isn't LM Studio (or the call fails), the cache stays
// empty and callers fall through to the existing JSON config + family-fallback
// heuristics.
// ---------------------------------------------------------------------------

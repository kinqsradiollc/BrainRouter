 ...
 * Every snapshot is one no-follow open/read/fstat/lstat operation; later
 * decisions compare those immutable in-memory bytes plus device and inode.
 */
...
export function beginWorkspaceManifestClaim(
  ...
  const receipt: WorkspaceManifestClaimReceipt = {
    version: 1,
    phase: 'prepared',
    ownerLeaseExpiresAt: Date.now() + OWNER_LEASE_MS,
    target,
    claim,
    token,
...
export function recoverInterruptedWorkspaceManifestClaim(workspaceRoot: string): void {
  ...
  try {
    ...
    for (const name of entries) {
      ...
      if (!receipt || transactionOwnerIsActive(receipt)) continue;
      ...
      try {
        ...
        // Another recoverer may have linked the owned inode and retired its
      ...
      // Both names must still resolve to the originally snapshotted inode and
      // bytes. Sequential path lookup cannot make a different inode satisfy
      // these device/inode/content comparisons.
      if (!snapshotsAreSameVersion(claimed, restored) ||
          (claimAfterLink.existed && !snapshotsAreSameVersion(claimed, claimAfterLink))) {
...
function readReceipt(
  ...
  if (receipt.version !== 1 ||
      ...
      typeof receipt.ownerLeaseExpiresAt !== 'number' ||
      !Number.isSafeInteger(receipt.ownerLeaseExpiresAt) || receipt.ownerLeaseExpiresAt < 0 ||
...
function manifestClaimOwnsReplacement(
  ...
  // `claimed` and `current` are complete immutable snapshots. This proof does
  // not reopen either path after hashing it.
...
function snapshotRegularFile(target: string, maxBytes: number, label: string): RegularFileSnapshot {
  ...
  try {
    ...
    const pathAfterOpen = fs.lstatSync(target);
    if (!opened.isFile() || pathAfterOpen.isSymbolicLink() || !pathAfterOpen.isFile() ||
        !sameFilesystemEntry(opened, pathAfterOpen) || opened.size > maxBytes) {
...
/** Hard-link cleanup changes ctime without changing the owned file version. */
...
function transactionOwnerIsActive(receipt: WorkspaceManifestClaimReceipt): boolean {
  const { token } = receipt;
  const ownerPid = Number(token.slice(0, token.indexOf('.')));
  if (ownerPid === process.pid) return activeClaimTokens.has(token);
  // The filesystem-only claim section is expected to complete in milliseconds.
  // A generous persisted lease protects a live peer while bounding stale PID
  // reuse: an unrelated process can delay recovery for at most this interval.
  if (Date.now() >= receipt.ownerLeaseExpiresAt) return false;
  try {
...
function assertSafeReceiptDirectory(directory: string): void {
  ...
  if ((stat.mode & 0o077) !== 0) {
    throw new Error(`Unsafe workspace manifest transaction directory permissions: ${directory}`);
  }
  const currentUser = process.getuid?.();
  if (currentUser !== undefined && stat.uid !== currentUser) {
    throw new Error(`Unsafe workspace manifest transaction directory owner: ${directory}`);
  }
}
...
/** Concurrent cleanup is idempotent; every other unlink failure stays fatal. */

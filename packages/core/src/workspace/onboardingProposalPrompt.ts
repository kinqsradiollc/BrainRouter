...
export function buildWorkspaceOnboardingPrompt(
  ...
  const description = truncateUtf8(input.description?.trim() ?? '', ONBOARDING_DESCRIPTION_MAX_BYTES);
...
function repositoryEvidence(scan: RepositoryScanSummary): string {
  const header = truncateUtf8(JSON.stringify({
    markers: scan.markers.slice(0, REPOSITORY_SCAN_ROOT_MARKERS.length),
    stats: boundedStats(scan.stats),
    stoppedBy: scan.stoppedBy.slice(0, 6),
  }), 8 * 1024);
  ...
  for (const { file } of files) {
    ...
    const safePath = truncateUtf8(file.path, 512);

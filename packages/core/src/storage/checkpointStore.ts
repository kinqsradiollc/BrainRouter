...
function inflightPath(ws: string, sessionKey: string): string {
  return path.join(getStateDir(ws), 'checkpoints', `${encodeKey(sessionKey)}.inflight.json`);
}
function offlinePath(ws: string, sessionKey: string): string {
  return path.join(getStateDir(ws), 'checkpoints', `${encodeKey(sessionKey)}.offline.json`);
}

...
test('a custom policy is obeyed in full — the rule is data, not a constant', () => {
  const policy: MeetingRetryPolicy = { maxAttempts: 2, baseDelayMs: 100, factor: 10, maxDelayMs: 500 };
...
test('D7 — the drain returns exactly the segments due right now', () => {
  ...
  const due = retryableSegments(session, { nowMs: T0_MS + 5_000 });
  assert.deepEqual(due.map((segment) => segment.index), [1], 'segment 2 is still inside its backoff window');
  const laterDue = retryableSegments(session, { nowMs: T0_MS + 60_000 });
  assert.deepEqual(laterDue.map((segment) => segment.index), [1, 2]);
});

test('D6 — a discarded meeting is never retried; its audio is gone', () => {
  const session = discardCapture(failed(1));
  assert.deepEqual(retryableSegments(session, { nowMs: T0_MS + 60_000 }), []);
});
...
test('exhaustion is reported for the session so a host knows when to stop draining', () => {
  ...
  assert.equal(retriesExhausted(clean), false, 'a session with no failures has nothing to exhaust');
  assert.equal(retriesExhausted(failed(1), { nowMs: T0_MS + 60_000 }), false);
  assert.equal(retriesExhausted(failed(DEFAULT_MEETING_RETRY_POLICY.maxAttempts), { nowMs: T0_MS + 60_000 }), true);
});

...
function withInterruptedClaim(
  ...
  try {
    ...
    const receipt = JSON.parse(fs.readFileSync(transaction.receiptPath, 'utf8')) as {
      ownerLeaseExpiresAt: number;
    };
    assert.ok(receipt.ownerLeaseExpiresAt > Date.now());
    assert.ok(receipt.ownerLeaseExpiresAt <= Date.now() + 5 * 60 * 1000);
...
test('manifest loading restores an interrupted owned claim', () => {
  withInterruptedClaim(({ workspace, target, claim, receiptPath }) => {
    assert.equal(loadWorkspaceManifest(workspace)?.profile, 'engineering');
...
test('manifest recovery converges when another recoverer wins link and cleanup races', () => {
  withInterruptedClaim(({ workspace, target, claim, receiptPath }) => {
...
test('manifest recovery preserves a different creator that wins the link race', () => {
  withInterruptedClaim(({ workspace, target, claim, receiptPath }) => {
    const originalLink = fs.linkSync;
    let racedLink = false;
    fs.linkSync = ((existingPath, newPath) => {
...
test('manifest recovery rejects an accessible receipt directory', { skip: process.platform === 'win32' }, () => {
  withInterruptedClaim(({ workspace, receiptPath }) => {
    fs.chmodSync(path.dirname(receiptPath), 0o777);
    assert.throws(
      () => recoverInterruptedWorkspaceManifestClaim(workspace),
      /Unsafe workspace manifest transaction directory permissions/,
    );
  });
});

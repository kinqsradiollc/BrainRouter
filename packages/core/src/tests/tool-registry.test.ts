...
test('CODEX-TOOL-REGISTRY exposure tiers match the reviewed allowed sets exactly', () => {
  ...
  // Write adds exactly the structured file tools (plus ADR-056's diagram_render, design_fidelity, and design_variants,
  ...
  assert.deepEqual([...write].filter((t) => !read.has(t)).sort(), ['apply_patch', 'design_fidelity', 'design_variants', 'diagram_render', 'edit_file', 'notebook_edit', 'write_file']);

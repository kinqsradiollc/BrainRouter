 ...
 * the sibling `blockEditing` helper, so a keystroke's landing place is a tested
...
import { blockKeyIntent, stopsWindowShortcut, type BlockIntent } from './blockKeymap.js';
import {
  caretEntryPoint, mergeRowRects, type RowRect, type VisualRowGeometry,
} from './visualRows.js';
import { redoRoute, undoRoute } from './pageUndo.js';
import {
...
export interface BlockEditorProps {
  ...
  /** Resolve true only after the host has granted this block's edit lease. */
...
export function BlockEditor(props: BlockEditorProps): React.ReactElement {
  ...
  const [leaseState, setLeaseState] = useState<'idle' | 'pending' | 'granted' | 'refused'>('idle');
  const leaseStateRef = useRef<'idle' | 'pending' | 'granted' | 'refused'>('idle');
  const leaseGenerationRef = useRef(0);
  const pendingLeaseRef = useRef<Promise<boolean> | null>(null);

  const updateLeaseState = useCallback((next: 'idle' | 'pending' | 'granted' | 'refused') => {
    leaseStateRef.current = next;
    setLeaseState(next);
  }, []);
  const editorReadOnly = readOnly || leaseState !== 'granted';
  ...
  useEffect(() => {
    const next = readOnly ? text : textToShow({
  ...
  const releaseLease = useCallback(() => {
    const held = leaseStateRef.current === 'granted';
    leaseGenerationRef.current += 1;
    pendingLeaseRef.current = null;
    updateLeaseState('idle');
    focusedRef.current = false;
    if (held) props.onBlur();
  }, [props.onBlur, updateLeaseState]);

  const requestLease = useCallback((): Promise<boolean> => {
    if (readOnly) return Promise.resolve(false);
    if (leaseStateRef.current === 'granted') return Promise.resolve(true);
    if (pendingLeaseRef.current) return pendingLeaseRef.current;
    const generation = leaseGenerationRef.current + 1;
    leaseGenerationRef.current = generation;
    updateLeaseState('pending');
    const pending = props.onFocus()
      .then((granted) => {
        if (leaseGenerationRef.current !== generation) {
          if (granted) props.onBlur();
          return false;
        }
        pendingLeaseRef.current = null;
        if (!granted) {
          updateLeaseState('refused');
          return false;
        }
        updateLeaseState('granted');
        focusedRef.current = true;
        rootRef.current?.focus({ preventScroll: false });
        return true;
      })
      .catch(() => {
        if (leaseGenerationRef.current === generation) {
    ...
    pendingLeaseRef.current = pending;
    return pending;
  }, [props.onBlur, props.onFocus, readOnly, updateLeaseState]);

  // A renewal failure or a holder arriving invalidates the local draft. Adopt
  // the refreshed server/conflict projection immediately and close every affordance
  // that could otherwise imply the fenced draft had won.
  useEffect(() => {
    if (!readOnly) return;
    const held = leaseStateRef.current === 'granted';
    leaseGenerationRef.current += 1;
    ...
    if (held) props.onBlur();
  }, [props.onBlur, readOnly, text, updateLeaseState]);
   ...
   *
   * Measured here because only the DOM knows; judged in the sibling `visualRows`
  ...
  }, [editorReadOnly, focusRequest?.token, readOnly, requestLease]);
  ...
  const stepHistory = useCallback((direction: 'undo' | 'redo') => {
    ...
    // F4 — the routing judgement is in the sibling `pageUndo`, not here: this
  ...
  const onBeforeInput = useCallback((event: InputEvent) => {
    const root = rootRef.current;
    if (!root) return;
    if (editorReadOnly) { event.preventDefault(); return; }
    ...
    event.preventDefault();
  }, [applyEdit, askForSplit, editorReadOnly, flushPush, insert, onIntent, readSelection, refreshMenus, stepHistory]);
  ...
  }, [askForSplit, closeMenu, editorReadOnly, flushPush, insert, kind, onIntent, pickMenuRow, readGeometry, readSelection, schedulePush, searchMentions, setCaret, stepHistory]);
  ...
  return (
    <div className="notes-editor">
      <div
        ...
        onFocus={() => {
          if (leaseStateRef.current !== 'granted') {
            focusedRef.current = false;
            void requestLease();
            return;
          }
        ...
        onBlur={() => {
          const granted = leaseStateRef.current === 'granted';
          focusedRef.current = false;
          if (granted && !readOnly) flushPush();
          setMenu(null);
          setToolbar(null);
          releaseLease();
        }}
        onPointerDown={(event) => {
          if (readOnly || leaseStateRef.current === 'granted') return;
          event.preventDefault();
          rootRef.current?.focus({ preventScroll: false });
          void requestLease();
        }}
...
/**
 * A `DOMRectList` as the plain numbers the sibling `visualRows` helper decides on.
 *

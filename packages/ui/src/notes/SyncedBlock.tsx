...
import { placeholderFor } from './notesView.js';
...
export interface SyncedBlockOps {
  ...
  /** Mirrored rows lease their source block before becoming writable. */
  ...
  useEffect(() => {
    ...
    if (!ops.liveReferences) {
      setState({
        ...
        note: 'This host cannot read synced blocks.',
      });
      return;
    }
    let cancelled = false;
    void ops.liveReferences.readSynced(blockId).then((next) => {
      if (cancelled) return;
    ...
    <div className="notes-synced" data-status={state?.status ?? 'loading'}>
      {ready ? (
        <div className="notes-synced-body">
          {state!.rows.map((row) => (
            <SyncedRow key={row.id} row={row} readOnly={readOnly} ops={ops} />
          ))}
          {state!.rows.length === 0 ? (
 ...
 */
function SyncedRow({ row, readOnly, ops }: {
  row: SyncedRowDto;
  readOnly: boolean;
  ops: SyncedBlockOps;
}): React.ReactElement {
    ...
    <div className="notes-synced-row" data-kind={row.kind} style={{ paddingLeft: syncedIndent(row.depth) }}>
      ...
      {syncedRowIsProse(row.kind) ? (
        <BlockEditor
          ...
          refLabels={{}}
          onOpenRef={() => {}}
          onText={(next) => ops.setText(row.id, next)}
          onIntent={() => {}}
          onFocus={() => ops.beginEdit(row.id)}
          onBlur={() => ops.endEdit(row.id)}

...
**Read [`AGENT.md`](AGENT.md) — it is the canonical contributor manual for this repository** (read order, directory map, build/test commands, git/PR conventions, ADR process, and the scenario → skill map). This file exists so agents that look for `AGENTS.md` (e.g. Codex) find the same instructions; it intentionally contains only the pointers and the hard rules.

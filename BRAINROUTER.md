...
## 🗄️ Database Schema & SQLite WAL Table Layout

The persistent store resides in [sqlite.ts](./mcp/src/memory/store/sqlite.ts) (`mcp/memory.db`) running SQLite in WAL mode with the `sqlite-vec` extension. Below are the key tables and indexes:
...
erDiagram
    ...
    users ||--o{ l0_turns : generates
    users ||--o{ l1_memories : possesses
    users ||--o{ skill_activations : updates
    users ||--o{ l2_scenes : clusters
    users ||--o{ l3_personas : defines
    users ||--o{ hook_registrations : triggers
```

### 2. Indexes & Shadow Tables
*   **Vector Search Table (`l1_vec`):** Virtual table matching `vec0` dimensions (cosine metric) mapping `record_id` and embedding coordinates.
*   **Full-Text Search Table (`l1_fts`):** Virtual table matching FTS5 mapping `record_id`, `content`, `type`, and `skill_tag` to support fast token keyword lookup.
*   **FTS5 Triggers:** Triggers created to automatically synchronize changes to `l1_memories` directly to `l1_fts` and the vector index.

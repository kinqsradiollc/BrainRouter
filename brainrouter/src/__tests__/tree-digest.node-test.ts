...
async function makeParent(store: PostgresMemoryStore, userId: string) {
  const c1 = await store.appendTreeNode(userId, { kind: "topic", level: 0, summaryMd: "child one: auth route leaks the api key" });
  const c2 = await store.appendTreeNode(userId, { kind: "topic", level: 0, summaryMd: "child two: recall fuses fts and vector" });
  const parent = await store.appendTreeNode(userId, { kind: "global", level: 1, summaryMd: "deterministic: child one... child two..." });
  await store.setTreeParent([c1.id, c2.id], parent.id);
  return parent;
}
...
test("digestTreeNodes: replaces the parent summary with the LLM output", async () => {
  ...
  try {
    const parent = await makeParent(store, "u1");
    ...
    assert.match((await store.getTreeNode(parent.id))!.summaryMd, /leaks the API key; recall fuses FTS/);
  } finally {
    await cleanup();
  }
});
...
test("digestTreeNodes: LLM failure keeps the deterministic summary (graceful)", async () => {
  ...
  try {
    const parent = await makeParent(store, "u1");
    const before = (await store.getTreeNode(parent.id))!.summaryMd;
    const r = await digestTreeNodes({
      userId: "u1",
      nodeIds: [parent.id],
      store: store as unknown as IMemoryStore,
    ...
    assert.equal((await store.getTreeNode(parent.id))!.summaryMd, before, "deterministic summary preserved on LLM failure");
  } finally {
    await cleanup();
  }
});
...
test("digestTreeNodes: skips foreign-user nodes + childless nodes", async () => {
  ...
  try {
    const parent = await makeParent(store, "u1");
    const childless = await store.appendTreeNode("u1", { kind: "global", level: 1, summaryMd: "no children" });
...
test("tree_sealer auto-chains a tree_digest job for the sealed parent", async () => {
  ...
  try {
    ...
    const jobs = (await store.listMemoryJobs({ kind: "tree_digest", status: ["pending", "running"] })) as Array<{ input?: { nodeIds?: string[]; userId?: string } }>;

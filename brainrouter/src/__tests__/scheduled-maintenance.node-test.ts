...
const pendingJobs = async (store: PostgresMemoryStore, kind: string, userId: string) =>
  (await store.listMemoryJobs({ kind, status: ["pending", "running"] })).filter((j: any) => j.input?.userId === userId);

test("maintenance: vault on pass 0, blackboard only when pending, throttle + off-gate", async () => {
  ...
  try {
    ...
    const r0 = await engine.enqueueScheduledMaintenance(true);
    ...
    assert.equal((await pendingJobs(store, "vault_exporter", "u1")).length, 1, "u1 has a pending vault job");
    assert.equal((await pendingJobs(store, "blackboard_reconciler", "u1")).length, 0);

    // Throttle: an immediate, non-forced call does nothing.
    assert.equal((await engine.enqueueScheduledMaintenance(false)).skipped, true, "throttled within the interval");

    // Stage a pending candidate → next forced pass enqueues the reconciler for u1.
    await engine.stageBlackboardCandidates("u1", [{ score: 0.9, candidate: { content: "fact A", type: "codebase_fact" } }]);
    const r1 = await engine.enqueueScheduledMaintenance(true); // pass 1
    ...
    assert.equal((await pendingJobs(store, "blackboard_reconciler", "u1")).length, 1, "u1 has a pending reconciler job");

    // Off-gate.
    const prev = process.env.BRAINROUTER_JOB_MAINTENANCE;
    process.env.BRAINROUTER_JOB_MAINTENANCE = "off";
    try {
      assert.equal((await engine.enqueueScheduledMaintenance(true)).skipped, true, "off-gate skips even when forced");
    } finally {
...
test("maintenance: enqueues vault for each active user; dedupes an in-flight job", async () => {
  const { store, engine, cleanup } = await createTestEngine();
  try {
    await store.createUser("u1", "br_k1", "U1", false);
    await store.createUser("u2", "br_k2", "U2", false);

    const r = await engine.enqueueScheduledMaintenance(true); // pass 0 → vault for each
    assert.ok(r.enqueued.vault_exporter >= 2, "vault enqueued for each user");
    assert.equal((await pendingJobs(store, "vault_exporter", "u1")).length, 1);
    assert.equal((await pendingJobs(store, "vault_exporter", "u2")).length, 1);
  } finally {
    await cleanup();
  }
});

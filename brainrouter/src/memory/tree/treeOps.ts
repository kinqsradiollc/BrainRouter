...
function tStore(engine: MemoryEngine): {
  appendTreeNode(userId: string, input: MemoryTreeNodeInput): Promise<MemoryTreeNode>;
  getTreeNode(id: string): Promise<MemoryTreeNode | null>;
  getTreeChildren(parentId: string): Promise<MemoryTreeNode[]>;
  getTreeRoots(userId: string, kind?: MemoryTreeKind): Promise<MemoryTreeNode[]>;
  setTreeParent(childIds: string[], parentId: string): Promise<void>;
  sealTreeNode(id: string): Promise<void>;
} | null {
...
export async function appendTreeLeaf(engine: MemoryEngine, userId: string, kind: MemoryTreeKind, summaryMd: string, sourceChunkIds: string[] = [], heatScore = 0): Promise<MemoryTreeNode | null> {
...
export async function summarizeBucket(engine: MemoryEngine, userId: string, childIds: string[], kind: MemoryTreeKind): Promise<MemoryTreeNode | null> {
  const store = tStore(engine);
  if (!store) return null;
  const children = (await Promise.all(childIds.map((id) => store.getTreeNode(id)))).filter((n): n is MemoryTreeNode => !!n);
  if (children.length === 0) return null;
  const parent = await store.appendTreeNode(userId, {
    kind,
  ...
  await store.setTreeParent(childIds, parent.id);
  for (const c of children) await store.sealTreeNode(c.id);
  return parent;
}
 ...
 */
export async function rollupGlobalTree(engine: MemoryEngine, userId: string): Promise<MemoryTreeNode | null> {
  if (!treeAutobuildEnabled()) return null;
  const store = engine.store as any;
  if (typeof store.getTreeRoots !== "function") return null;
  const roots = ((await store.getTreeRoots(userId, "global")) as MemoryTreeNode[]).filter((n) => !n.sealedAt);
...
export async function treeWalk(engine: MemoryEngine, userId: string, nodeId?: string, kind?: MemoryTreeKind): Promise<{ node: MemoryTreeNode | null; children: MemoryTreeNode[]; roots?: MemoryTreeNode[] }> {
  const store = tStore(engine);
  if (!store) return { node: null, children: [] };
  if (nodeId) {
    const node = await store.getTreeNode(nodeId);
    return { node, children: node ? await store.getTreeChildren(nodeId) : [] };
  }
  return { node: null, children: [], roots: await store.getTreeRoots(userId, kind) };
}

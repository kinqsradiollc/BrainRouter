---
name: hallmark
description: "Design skill for user-facing interfaces built on a user's own project — new pages and apps by default, plus audit, redesign, and study of a reference. Enforces structural variety so two different briefs do not come back as colour-swaps of one template. It does not govern work on BrainRouter itself."
hints:
  - "This skill applies to a user's own project. Work on BrainRouter's own surfaces follows brainrouter-rules/ instead, and that handbook wins wherever the two disagree."
  - "Choose the macrostructure from the brief before writing markup; two different briefs must not share one section rhythm."
  - "Score every artifact on the six pre-emit axes and revise anything below 3 before returning it."
  - "Never invent a metric, testimonial, logo, or customer count the user did not supply."
  - "Load the reference file a verb names before running it — references/study.md for study, references/anti-patterns.md for audit."
---
...
## Where this skill applies, and where it does not

This skill governs interfaces built **on a user's own project** — the app, site, or
component a user asked for. Work on BrainRouter's own surfaces (the desktop app, the
dashboard, the CLI) is governed by `brainrouter-rules/`, and that handbook wins wherever
the two disagree. BrainRouter's desktop is monochrome by deliberate decision, so this
skill's colour anchor, its twenty-one themes, and its custom-palette branch have nothing
to say about it.

Two documents telling an agent how to write UI is how they drift apart. This section is
the seam between them, stated here because a handbook the model never loads cannot
settle a conflict while it is generating. The same boundary is recorded from the other
side in `brainrouter-rules/09-docs-skills-and-plugins.md` § 7b.

> **Provenance.** Vendored from the MIT-licensed `hallmark` package (v1.1.0) published by
> Together AI. Its licence travels beside it as `LICENSE`, and the packaging step copies
> both into every package that ships the library. Everything below this line is upstream
> text: the local changes are this section, the frontmatter above, and one section
> heading renamed so the skill tools can address it.
...
| --- | --- |
| *(default)* | The user asked you to design or build something new. Follow the **Design workflow** below. |
...
Before entering the full Design workflow, **check scope**. If any of these fire, run the Component-scope flow instead — most day-to-day dev requests are component-shaped, not page-shaped, and the page-level apparatus (macrostructure, hero enrichment, footer archetype, project memory) is wrong for them.
...
If two signals fire, route component. If only the page flow fires (multi-section brief, "build me a landing page"), stay in Design workflow.
